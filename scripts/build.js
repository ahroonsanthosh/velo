...
// Critical CSS: the whole stylesheet is modest (~25KB) for a single-page
// site, so it's all inlined — this removes two render-blocking requests
// from the critical path entirely rather than splitting an artificial
// "above the fold" subset.
const inlineCss =
...
const svg = {
  ...
  olive: (cls = "") => `<svg class="${cls}" viewBox="0 0 200 70" fill="none" aria-hidden="true">
    <path d="M6 54 C 50 48, 90 32, 194 10" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"/>
...
function renderDiffs() {
  const icons = [svg.wheel("diff-card__icon"), svg.olive("diff-card__icon"), svg.pin("diff-card__icon")];
  return differentiators
    .map(
      (d, i) => `
    <article class="diff-card" data-reveal>
      ${icons[i] || svg.badge("diff-card__icon")}
  ...
  return menu.categories
    ...
    <div class="menu-cat" data-open="${i === 0 ? "true" : "false"}">
      <button class="menu-cat__trigger" aria-expanded="${i === 0 ? "true" : "false"}" aria-controls="panel-${cat.id}">
        <div class="menu-cat__thumb">${picture(thumbName, { sizes: "64px", alt: "" })}</div>
...
function buildJsonLd() {
  ...
  const menuItemsFlat = menu.categories.flatMap((c) => c.items.map((i) => `${i.name} ${i.price}`));
...
<main id="main">
  ...
  <section class="hero">
    <div class="container">
      <div class="hero__copy">
        ...
        <h1 class="hero__title">Eggs, coffee,<br>and a bike on the wall<span class="accent"> — Velo<svg viewBox="0 0 160 12"><path d="M2 8 C 40 2, 120 2, 158 8"/></svg></span></h1>
        ...
        <div class="hero__ctas">
          <a class="btn btn--primary" href="${business.phoneHref}">${svg.phone()}Call<span class="btn__detail"> ${business.phone}</span></a>
          <a class="btn btn--ghost" href="https://www.google.com/maps/search/?api=1&query=${business.address.mapQuery}" target="_blank" rel="noopener">${svg.pin()}Get Directions</a>
        </div>
        <div class="hero__badge">${svg.wheel("")}<span data-open-pill-hero>Open ${DAY_KEY_TODAY_FALLBACK.label} today</span></div>
      </div>
  ...
  <section class="gallery" aria-labelledby="gallery-h">
    <div class="container">
      <div class="gallery__head" data-reveal>
        <div>
  ...
  <section class="hours-location" id="hours" aria-labelledby="hl-h">
    <div class="container">
      <div class="hl__head" data-reveal>
        ...
        <button class="wheel-toy" type="button" aria-label="Spin the wheel, purely for fun">
          ${svg.wheel("")}
      ...
      <div class="hl__grid">
        ...
        <div class="ticket" data-reveal>
          ...
          <div class="hl__actions">
            <a class="btn btn--outline-navy" style="color:var(--paper);border-color:rgba(251,248,241,0.5)" href="https://www.google.com/maps/search/?api=1&query=${business.address.mapQuery}" target="_blank" rel="noopener">${svg.pin()}Get Directions</a>
            <a class="btn btn--outline-navy" style="color:var(--paper);border-color:rgba(251,248,241,0.5)" href="${business.phoneHref}">${svg.phone()}Call the cafe</a>
          </div>
        </div>
      </div>
    </div>
